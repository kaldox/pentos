...
pentos policy setup --no-bruteforce --exploitation   # Programm-Regeln, z.B. Bug-Bounty-Scope
pentos policy setup                  # interaktiv durch alle Fragen
pentos policy show                   # aktuelle Programm-Regeln anzeigen
pentos policy clear                  # Programm-Regeln entfernen
...
pentos scan import-bloodhound export/ --host 1   # oder entpackter Ordner, an einen Host gebunden
...
pentos run hydra 10.10.10.10 --proto ssh             # Wordlists automatisch aus dem Projekt
pentos run hydra 10.10.10.10 --proto ssh --userlist eigene_users.txt --passlist eigene_pass.txt
pentos run hydra 10.10.10.10 --proto http-post-form --proto-extra "/login.php:user=^USER^&pass=^PASS^:F=Login failed"
pentos run nxc-smb 10.10.10.10       # netexec SMB, ebenfalls mit Projekt-Wordlists (kein --proto nötig)
...
> Sind Programm-Regeln gesetzt (`pentos policy setup`), sperren sie passende
> Kategorien zusätzlich mit klarer Fehlermeldung (Override: `--force`).
